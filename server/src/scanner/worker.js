...
const normalizeCfg = (c) => {
  ...
  return {
    ...
    atrF: Math.max(0.05, parseFloat(x.atrF ?? 2) || 2),
    ...
    minRiskPct: Math.max(0, parseFloat(x.minRiskPct ?? 0.004) || 0.004),
    maxRiskPct: Math.max(0.001, parseFloat(x.maxRiskPct ?? 0.06) || 0.06),
    maxTpPct: Math.max(0, parseFloat(x.maxTpPct ?? 0) || 0),
    ...
    rejectSampleLimit: clamp(parseInt(x.rejectSampleLimit ?? 100, 10) || 100, 1, 500),
...
const normalizeRejectKey = (reason) =>
  String(reason || "UNKNOWN_REJECT").trim() || "UNKNOWN_REJECT";

const buildRejectSample = (meta = {}) => ({
  symbol: String(meta.symbol || "").trim() || null,
  tf: String(meta.tf || "").trim() || null,
  side: String(meta.side || "").trim() || null
});

const bumpReject = (reason, meta = {}) => {
  if (!CFG?.rejectDebug) return;

  const key = normalizeRejectKey(reason);
  REJECT_COUNTS.set(key, (REJECT_COUNTS.get(key) || 0) + 1);

  const sample = buildRejectSample(meta);
  if (!sample.symbol || !sample.tf) return;

  const existing = REJECT_SAMPLES.get(key) || [];

  if (existing.length < CFG.rejectSampleLimit) {
    existing.push(sample);
    REJECT_SAMPLES.set(key, existing);
  }
};
...
const flushRejectSummary = (force = false) => {
  ...
  for (const [key, value] of REJECT_SAMPLES.entries()) {
    samples[key] = value;
  }
